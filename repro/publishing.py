...
V0_HARDENING_STEPS = [
    ...
    "Repeat a fully blind non-admin hidden-path condition at n=10 before making that the headline rather than a breadth check.",
...
def render_report(run_results: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "Short version: this is a qualitative v0 snapshot and first finding, not a statistical benchmark. It records a small, repeatable evidence loop for Drupal CMS/Haven so the team can inspect what agents can verify today and what the running site still needs to expose.",
        "",
        ...
        "",
        "In the headline run on stock Drupal CMS / Haven, each run asked the agent to judge two hidden path claims. Ten runs produced 20 hidden disabled-View path judgments per arm.",
        "",
        ...
        "",
        "Initial non-Claude evidence shows the same pattern: OpenAI Codex (gpt-5.5) judged 6/6 hidden claims safe with Drush-only inspection and flagged 6/6 with site self-description. That is encouraging breadth evidence, not yet a claim across model providers.",
        "",
