...
| --- | --- | --- | --- | --- | --- | --- |
| Site self-description changed agent behavior in one Drupal path-safety task. | Hidden disabled-View path judgment. | Stock Haven: 10 runs per arm, 2 hidden judgments per run, 20 hidden judgments per arm. | Fully blind A/B, Drush-only vs bundled `site-architecture:path-owner` fixture. | `method/prompts/assess.alias_safety.fully_blind.md`; stock Drupal CMS (Haven testbed profile). | `evidence/experiments/alias-safety-haven-n10-fullyblind-v0/model-ab-results.json`; `evidence/experiments/alias-safety-haven-n10-fullyblind-v0/model-ab-FINDING.md`. | Broad Drupal agent readiness, statistical power, production module readiness, or model-provider generality. |
...
- The equipped arm uses a bundled site-description fixture command that exposes
  ...
  collector. The 20/20 equipped result shows that surfaced Drupal state changed
