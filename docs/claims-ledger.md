...
| Initial Codex evidence points in the same direction. | Hidden disabled-View path judgment. | Codex: n=3 runs per arm, 6 hidden judgments per arm. | Fully blind A/B on stock Haven. | `method/prompts/assess.alias_safety.fully_blind.md`; stock Drupal CMS/Haven. | `evidence/experiments/alias-safety-codex-fullyblind-v0/model-ab-results.json`. | Provider-general behavior or a stable non-Claude baseline. |
