...
- The equipped arm uses a prototype command that exposes the same path-ownership
  predicate the evaluator grades through an independent collector. The 20/20
  equipped result shows that surfaced Drupal state changed the agent's judgment
  in this task; it is not independent proof of model capability or resolver
  completeness.
